...
2. **Determinate Nix**: Use `determinateNix.enable = true` via the official module (`modules/darwin/nix-storage.nix`) — do not manually set `nix.enable = false`, the module handles this automatically; use `nix` (Determinate), not `nix-env` or `nix-channel`
